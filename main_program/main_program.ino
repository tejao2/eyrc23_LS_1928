...
// Detection Pins
#define curr_out_1 A6
#define curr_out_2 A7
#define buzzer 9
#define r_led A1
#define g_led A0
...
void detection() {
  ...
  if (check1 > 518 || check2 > 518)
    safe();
  else if (check1 < 503 || check2 < 503)
   {
    damaged();
...
void loop() {
  ...
  //Serial.println(DC_Encoder.read());

  // Timing calculation
  t = millis();
  roll_vel_err = (roll_pos_err - roll_pos_err_prev) / (t - t_prev);
  yaw_vel_err = (yaw_pos_err - yaw_pos_err_prev) / (t - t_prev);
  t_prev = t;
  yaw_pos_err_prev = yaw_pos_err;
  roll_pos_err_prev = roll_pos_err;
  ...
  //u = 27 * roll_pos_err + 0.1 * roll_itgl_err + 0.009 * roll_vel_err - 0.39 * yaw_pos_err - 0.07 * yaw_vel_err - 0.0003 * yaw_itgl_err;
  u = 27 * roll_pos_err + 0.1 * roll_itgl_err + 0.009 * roll_vel_err - 0.39 * yaw_pos_err - 0.1 * yaw_vel_err - 0.00035 * yaw_itgl_err;
